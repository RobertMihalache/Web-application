...
$_SESSION['input']=test_input($_POST["search"]);
}
$input=$_SESSION['input'];
$sql="SELECT * FROM servere WHERE numeserver='$input' OR ip='$input' OR locatie='$input' OR asignat='$input'ORDER BY id";
    ...
    while($row = mysqli_fetch_assoc($result)){
		...
		elseif(isset($_POST['Submit']))
		{
			...
			?>
				...
				<form method="post" ><input type="hidden" name="id" value="<?php echo $id[$i];?>"><input type="submit" class="btn btn-default btn-block w3-yellow" value="Delete" name="delete">
				</form>
				</td><td>
				<button class="btn btn-default btn-block w3-purple" data-toggle="modal" data-target="#myModal<?php echo $id[$i];?>">Edit</button>
